...
interface RebuildManifestRequest {
  ideId: string
  enableLLMFallback?: boolean
  sampleSize?: number
}

interface RebuildManifestResponse {
  message: string
  data: {
    ideId: string
    ideName: string
    manifest: IDEManifest
    previousManifest: IDEManifest | null
  }
}

export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse<RebuildManifestResponse | { error: string }>
) {
  if (req.method !== 'POST') {
  ...
  const { ideId, enableLLMFallback = true, sampleSize = 50 } = req.body as RebuildManifestRequest
  ...
  try {
    ...
    const { data: ideData, error: ideError } = await supabaseAdmin
      .from('ides')
      .select('id, name, status, manifest')
      .eq('id', ideId)
      .maybeSingle()
    ...
    // Store previous manifest if it exists
    const previousManifest = ideData.manifest as IDEManifest | null
    ...
    res.status(500).json({
      error: error instanceof Error ? error.message : 'Failed to rebuild manifest'
    })
  }
}
