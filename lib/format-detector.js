export class FormatDetector {
    constructor(options) {
        var _a, _b;
        this.options = {
            enableLLMFallback: (_a = options === null || options === void 0 ? void 0 : options.enableLLMFallback) !== null && _a !== void 0 ? _a : true,
            minConfidence: (_b = options === null || options === void 0 ? void 0 : options.minConfidence) !== null && _b !== void 0 ? _b : 60
        };
    }
    async detectFormat(ideId, documentation) {
        const scores = [];
    ...
    analyzeFileExtensions(documentation) {
        ...
        const scores = [
            { format: 'json', score: jsonScore },
            { format: 'markdown', score: markdownScore },
            { format: 'xml', score: xmlScore }
        ];
    ...
    analyzeCodeFences(documentation) {
        ...
        const scores = [
            ...
            { format: 'markdown', score: markdownScore },
            { format: 'xml', score: xmlScore },
            { format: 'plaintext', score: plaintextScore }
        ];
        const topScore = scores.sort((a, b) => b.score - a.score)[0];
        return {
            format: topScore.format,
    ...
    analyzeJsonSchemas(documentation) {
        const text = documentation.toLowerCase();
        const methods = [];
        let score = 0;
    ...
    analyzeMarkdownStructure(documentation) {
        const methods = [];
        let score = 0;
        // Count headings
    ...
    analyzeCliPatterns(documentation) {
        const text = documentation.toLowerCase();
        const methods = [];
        let score = 0;
    ...
    async classifyWithLLM(ideId, documentation) {
